...
class RSGAN(SocialRecommender,DeepRecommender):
    ...
    def social_link_jaccard_similarity(self, u1, u2):
        u1_friends = set(self.social.followees[u1]).union(set(self.social.followers[u1]))
        u2_friends = set(self.social.followees[u2]).union(set(self.social.followers[u2]))
        intersection = u1_friends.intersection(u2_friends)
        union = u1_friends.union(u2_friends)
        return len(intersection)/len(union) if len(union) > 0 else 0
    
    def round_robin_to_k_no_repetitions(self, l1, l2):
        l1, l2 = l1[:], l2[:]
        res = []
        k = max(len(l1), len(l2))
        curr_list = l1
        while len(res) < k:
            if len(curr_list) > 0:
                candidate = curr_list.pop(0)
                while candidate in res and len(curr_list) > 0:
                    candidate = curr_list.pop(0)
                if candidate not in res:
                    res.append(candidate)
            curr_list = l2 if curr_list is l1 else l1
        return res
    ...
    def computeSimilarity(self):
        ...
        for i,user1 in enumerate(self.positive):
            ...
            for user2 in self.positive:
                if user1 <> user2:
                    ...
                    jaccard_sim = self.social_link_jaccard_similarity(user1, user2)
                    uSim_cosine.append((user2, cosine_sim))
                    uSim_social.append((user2, jaccard_sim))
            fList_cosine = sorted(uSim_cosine, key=lambda d: d[1], reverse=True)[:topK]
            fList_social = sorted(uSim_social, key=lambda d: d[1], reverse=True)[:topK]
            # self.pTopKSim[user1] = [item[0] for item in fList_cosine]
            # self.pTopKSim[user1] = [item[0] for item in fList_social]
            self.pTopKSim[user1] = [item[0] for item in self.round_robin_to_k_no_repetitions(fList_cosine, fList_social)]
        ...
        for i,user1 in enumerate(self.negative):
            ...
            fList_cosine = sorted(uSim_cosine, key=lambda d: d[1], reverse=True)[:topK]
            fList_social = sorted(uSim_social, key=lambda d: d[1], reverse=True)[:topK]
            # self.nTopKSim[user1] = [item[0] for item in fList_cosine]
            # self.nTopKSim[user1] = [item[0] for item in fList_social]
            self.nTopKSim[user1] = [item[0] for item in self.round_robin_to_k_no_repetitions(fList_cosine, fList_social)]
